...
static void getQuaternion_(float quater[4]);
...
/**
  ...
  * @usage  call in main() function
	*/
//由mpu数据得到四元数。AHRS融合？TODO===用DMP完成
...
void imu_coordinate_astimate(void)
{
	// 获得没有数据融合的四元数
	float quater[4];
	float gx,gv,gz;
	getQuaternion_(quater);
	//重力加速向量坐标变换
	gx = 2.0f*(quater[1]*quater[3] - quater[0]*quater[2]);
	gy = 2.0f*(quater[0]*quater[1] + quater[2]*quater[3]);
	gz = quater[0]*quater[0] - quater[1]*quater[1] - quater[2]*quater[2] +quater[3]*quater[3]; //用1代入
	// 方向估计
...
static void getQuaternion_(float quater[4])
{
	...
	halfT       = ((float)(now_update - last_update) / 2000.0f);
	last_update = now_update;
	tempq0 = quat[0] + (-quat[1]*gx - quat[2]*gy - quat[3]*gz) * halfT;
	tempq1 = quat[1] + (quat[0]*gx + quat[2]*gz - quat[3]*gy) * halfT;
	tempq2 = quat[2] + (quat[0]*gy - quat[1]*gz + quat[3]*gx) * halfT;
	tempq3 = quat[3] + (quat[0]*gz + quat[1]*gy - quat[2]*gx) * halfT;  

	/* normalise quaternion */
	norm = inv_sqrt(tempq0*tempq0 + tempq1*tempq1 + tempq2*tempq2 + tempq3*tempq3);
	quater[0]=quat[0]= tempq0 * norm;
	quater[1]=quat[1] = tempq1 * norm;
	quater[2]=quat[2] = tempq2 * norm;
	quater[3]=quat[3] = tempq3 * norm;
}
